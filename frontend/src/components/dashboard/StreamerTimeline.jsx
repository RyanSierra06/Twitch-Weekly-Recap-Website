               ...
               hover:border-[#ffc8fe] hover:bg-[#a471cf]/50`}
                    ...
                    >
                        <div className="font-bold text-white mb-1 flex items-center">
                            {format(day, 'EEE')}
                            {isCurrentDay && <span
                                className="ml-2 px-2 py-0.5 bg-[#ffc8fe] text-[#45323f] text-xs rounded-full font-bold">Today</span>}
                        </div>
                  ...
                  <span className="inline-flex items-center justify-center bg-[#fff]/10 rounded-full p-1 mb-1">
                    ...
                    <span
                        className="text-xs text-[#ffc8fe] font-semibold">{vod.view_count && vod.view_count.toLocaleString()}</span>
                  </span>
                                <div
                                    className="text-xs text-[#ffc8fe] mb-1">{formatClipDuration(parseTwitchDuration(vod.duration))}</div>
                            </div>
                        ) : (
